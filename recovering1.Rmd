...
read_delim("string_TFs.txt", delim = "\t", col_names = "text", skip = 1)%>%
  separate(text, into = c("critical_gene", "Gene2", "Direct", "Interaction", "shared_gene_num", "shared_genes"), sep = "\t")%>%
  mutate(shared_gene = str_split(shared_genes, pattern = ","))%>%
  unnest()%>%
  select(critical_gene, Gene2, shared_gene)%>%
  distinct()%>%
...
genes_lim1%>% #kritik gen yok
  mutate(interaction = ifelse(hgnc_symbol %in% TFs$shared_gene, "TF associated", NA),
         interaction = ifelse(hgnc_symbol %in% TFs$Gene2, "TFs", interaction),
